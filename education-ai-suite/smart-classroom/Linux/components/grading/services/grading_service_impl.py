...
def _linux_roots() -> list[dict[str, Any]]:
    """Enumerate Linux root locations for the UI directory picker: `/`, the
    user's home, `/home`, and any mounted volumes under `/media` and `/mnt`."""
    roots: list[dict[str, Any]] = []
    seen: set[str] = set()

    def _add(name: str, p: Path):
        try:
            if p.is_dir():
                key = str(p)
                if key not in seen:
                    seen.add(key)
                    roots.append({"name": name, "path": key, "is_dir": True})
...
def list_directory(path: str | None = None) -> dict[str, Any]:
    ...
    directory picker. path=None returns the Linux roots (`/`, home, mounted
    volumes). Only directories and *.pdf files are returned; file contents are
    ...
    if not path:
        return {"path": "", "parent": None, "entries": _linux_roots()}
