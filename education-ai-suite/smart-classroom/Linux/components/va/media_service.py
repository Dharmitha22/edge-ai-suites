...
class MediaService:
    ...
    def launch_server(self, timeout: float = 10.0) -> bool:
        ...
        try:
            ...
            self.process = subprocess.Popen(
                ...
                # Run MediaMTX in its own session/process group (os.setsid) so the
                # whole subtree can be signalled together via os.killpg().
    ...
    def stop_server(self) -> bool:
        ...
        try:
            ...
            # Try graceful shutdown first. The server was launched with
            # start_new_session=True, so it leads its own process group and we
            # send SIGTERM to the whole group to bring the MediaMTX subtree down.
            try:
