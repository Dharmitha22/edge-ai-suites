...
class VideoAnalyticsPipelineService:
    ...
    def _get_gstreamer_version(self) -> Optional[str]:
        """Read the installed GStreamer version on Linux.

        Prefers `gst-inspect-1.0 --version` (ships with GStreamer) and falls
        back to `pkg-config --modversion gstreamer-1.0`.
        """
        try:
            ...
            if result.returncode == 0:
                # Output looks like: "gst-inspect-1.0 version 1.24.0\nGStreamer 1.24.0 ..."
                for line in result.stdout.splitlines():
                    if "version" in line:
                        return line.strip().split()[-1]
        except (OSError, subprocess.SubprocessError) as e:
            self.logger.warning(f"gst-inspect-1.0 version lookup failed: {e}")
        ...
        try:
            ...
            self.logger.warning(f"pkg-config GStreamer version lookup failed: {e}")
    ...
    def _validate_file_with_discoverer(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error("gst-discoverer-1.0 not found")
            return "gst-discoverer-1.0 not found"
    ...
    def _launch_pipeline_internal(
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                bufsize=1,
                env=os.environ.copy(),
                # Start the child in its own session/process group so the whole
                # GStreamer subtree can be signalled together via os.killpg().
    ...
    def launch_pipeline(
        ...
        try:
            ...
            # Build full command
            command = ["gst-launch-1.0", "-e"] + pipeline_elements
    ...
    def stop_pipeline(self, pipeline_name: str, timeout: float = 10.0) -> bool:
        ...
        try:
            ...
            # Try graceful shutdown. The child was launched with
            # start_new_session=True, so it leads its own process group and we
            # send SIGTERM to the whole group to bring the GStreamer subtree down.
            try:
