...
// DEV-ONLY UI smoke test: render the UI shell without a running backend.
// When true, the backend-health and feature-config gates below are skipped so
// the SPA renders offline (API-backed panels will still error in the console).
// Set back to false (or delete) before committing / for real deployments.
const DEV_UI_PREVIEW = true;
...
const App: React.FC = () => {
  ...
  const openScreen = (screen: typeof activeScreen) => {
  ...
  if (!DEV_UI_PREVIEW && backendStatus === 'checking') {
    return (
  ...
  if (!DEV_UI_PREVIEW && backendStatus === 'unavailable') {
    return (
  ...
  if (!DEV_UI_PREVIEW && (featuresLoading || !featuresLoaded)) {
    return (
      <div className="app-loading">
        <div className="loading-content">
          <div className="app-spinner" />
  ...
  if (!DEV_UI_PREVIEW && featuresError) {
    return (
      <div className="app-error">
        <div className="error-content">
