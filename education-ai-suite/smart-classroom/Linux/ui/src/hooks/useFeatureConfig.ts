...
export function useFeatureConfig() {
  ...
  useEffect(() => {
    // `error` is part of the guard so a failed /features fetch does not
    // re-trigger the effect in an infinite retry loop (setError resets both
    // `loaded` and `loading` to false, which would otherwise re-run this).
